# JOV Plugin Architecture (Maintainer Memo)

Purpose: extend JOV capabilities without bloating core; ship tools + skills + config as reproducible units you can pin, test, and roll back. nix-jov shows the contract; JOV core should treat the same interface as first-class, even off-Nix.
...
## Interface Contract (reference implementation: nix-jov)
Every plugin artifact exposes the same fields (flake output `jovPlugin` today, but the shape is host-agnostic):
...
- Worktree: build and test plugins outside the core repo; point JOV at a local path source (e.g., `source = "path:/Users/you/code/my-plugin"`).
...
- Skills placement: skills land under `~/.jov*/workspace/skills/<plugin>/...` so you can inspect quickly; delete the workspace to fully reset cached skills.
...
```nix
programs.jov.instances.default.plugins = [
  { source = "github:J0VEBOT/nix-steipete-tools?dir=tools/summarize"; }
];
```
...
- Each follows the same contract: packages + skills; env/state declared via `needs`; enabled via config toggle; sources pinned (see nix-jov bundledPlugins mapping).
...
- Clean core: main JOV stays transport/model-focused; plugins carry integrations.
