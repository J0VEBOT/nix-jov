...
nix-jov is opinionated: **there should be one obvious way to deploy**.
...
- Node: JOV.app on macOS (connects over WebSocket)
...
JOV.app supports **Nix mode** (`OPENCLAW_NIX_MODE=1` or `defaults write ai.jov.mac jov.nixMode -bool true`).
...
You can check required permissions in `jovebot nodes status/describe` and then approve them once.

## Runtime state vs pinned config

Pinned / Nix-managed:
- `jov.json` (gateway config)
