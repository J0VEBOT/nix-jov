# RFC: JOV Plugin System — The Golden Path

- Date: 2026-01-11
- Status: Draft
- Audience: Peter (jov maintainer), nix-jov maintainers
...
**This RFC argues:** The nix-jov plugin model achieves both. It should become the golden path for extending jov.
...
- Changes to voice-call require jovebot releases
- Testing voice-call means testing all of jov
...
## The nix-jov Model: Bundle Everything
...
| Release cycle | Tied to jovebot | Independent |
...
nix-jov is a **plugin installer** that wires plugins into jov's plugin system:

```nix
# User's flake.nix
programs.jov.customPlugins = [
...
# Or enable bundled plugins (pinned in nix-jov):
programs.jov.bundledPlugins.summarize.enable = true;
programs.jov.bundledPlugins.oracle.enable = true;
```
...
- First-party toggles — curated plugins pinned in nix-jov

At activation time, nix-jov:
1. Resolves flake sources (remote or local) → builds binaries
2. Validates `requiredEnv` (fails if missing)
3. Creates state dirs
4. Installs plugins to `~/.jov/extensions/<plugin>/`
5. Writes `plugin.json` manifest for each
6. Symlinks skills to workspace
7. Adds binaries to PATH

**jov core sees all plugins** — it scans `~/.jov/extensions/`, reads manifests, knows what's installed. The difference is nix-jov does the install + validation at build time (deterministic, fail-fast), while non-Nix users do it manually or via npm.

**Same plugin system, different installers:**
- Nix users: nix-jov installs plugins declaratively
- Non-Nix users: `jovebot plugins install` or manual setup
- jovebot core: sees the same `~/.jov/extensions/` structure either way

**Local dev workflow:** Point at a local path, change code, rebuild, gateway picks up changes. No push/pull cycle. Same contract, local iteration. For non-Nix: symlink your plugin dir into `~/.jov/extensions/`.

### What nix-jov provides (golden path)
...
- `jovebot plugins install @jov/voice-call` — npm install to extensions dir
...
jovPlugin = {
  ...
  packages = [ self.packages.${system}.default ];
  needs = {
    stateDirs = [ ".config/jov-voice-call" ];
...
**First-party plugins** already exist — see [nix-steipete-tools](https://github.com/J0VEBOT/nix-steipete-tools/tree/main/tools):
...
All follow the same contract. All pinned in nix-jov. Enable with one line:
```nix
programs.jov.bundledPlugins.summarize.enable = true;
```

**Community plugins** (anyone can ship):
- Just a GitHub repo with `flake.nix` + `jovPlugin`
...
Your voice-call is great. But it shouldn't live in core forever. The nix-jov plugin model lets you:
- Ship it independently (faster iteration)
- Keep core lean (easier maintenance)
- Let community extend jovebot (ecosystem growth)
...
1. **Manifest format:**
   - Nix users: `jovPlugin` in `flake.nix` (already works)
   - Non-Nix users: `plugin.json` in plugin directory
   - **Suggested:** jovebot core defines `plugin.json` schema. Same fields as `jovPlugin`:
     ```json
     {
       "name": "voice-call",
       "skills": ["./skills/voice-call"],
...
2. **Discovery paths:**
   - **Suggested:** `~/.jov/extensions/<plugin>/` for user-installed, `.jov/extensions/<plugin>/` for project-local
   - jovebot core scans both paths at startup
   - nix-jov installs to the same paths — same structure, different installer

3. **First-party plugins for non-Nix:**
   - **Suggested:** `@jov/` npm scope, but don't invest heavily. Point people to Nix.
