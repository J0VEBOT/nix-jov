...
const outPath = argValue("--out") ?? path.join(process.cwd(), "nix/generated/jov-config-options.nix");
...
const loadSchema = async (): Promise<Record<string, unknown>> => {
  ...
  const schema = mod.JOVSchema;
  if (!schema || typeof schema.toJSONSchema !== "function") {
    console.error(`JOVSchema not found at ${schemaPath}`);
    process.exit(1);
  }
...
const renderOption = (key: string, schemaObj: JsonSchema, required: boolean, indent: string): string => {
  ...
  const output = `# Generated from upstream JOV schema. DO NOT EDIT.\n{ lib }:\nlet\n  t = lib.types;\nin\n{\n${body}\n}\n`;
