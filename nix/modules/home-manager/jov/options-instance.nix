{ lib, jovLib }:

{ name, config, ... }:
{
  options = {
    enable = lib.mkOption {
      type = lib.types.bool;
      default = true;
      description = "Enable this JOV instance.";
    };

    package = lib.mkOption {
      type = lib.types.package;
      default = jovLib.defaultPackage;
      description = "JOV batteries-included package.";
    };

    stateDir = lib.mkOption {
      type = lib.types.str;
      default = if name == "default"
        then "${jovLib.homeDir}/.jov"
        else "${jovLib.homeDir}/.jov-${name}";
      description = "State directory for this JOV instance (logs, sessions, config).";
    };

    workspaceDir = lib.mkOption {
      type = lib.types.str;
      default = "${config.stateDir}/workspace";
      description = "Workspace directory for this JOV instance.";
    };

    configPath = lib.mkOption {
      type = lib.types.str;
      default = "${config.stateDir}/jovebot.json";
      description = "Path to generated JOV config JSON.";
    };

    logPath = lib.mkOption {
      type = lib.types.str;
      default = if name == "default"
        then "/tmp/jov/jov-gateway.log"
        else "/tmp/jov/jov-gateway-${name}.log";
      description = "Log path for this JOV gateway instance.";
    };

    gatewayPort = lib.mkOption {
      type = lib.types.int;
      default = 18789;
      description = "Gateway port used by the JOV desktop app.";
    };

    gatewayPath = lib.mkOption {
      type = lib.types.nullOr lib.types.str;
      default = null;
      description = "Local path to JOV gateway source (dev only).";
    };

    gatewayPnpmDepsHash = lib.mkOption {
      type = lib.types.nullOr lib.types.str;
      default = lib.fakeHash;
      description = "pnpmDeps hash for local gateway builds (omit to let Nix suggest the correct hash).";
    };

    plugins = lib.mkOption {
      type = lib.types.listOf (lib.types.submodule {
        options = {
          source = lib.mkOption {
            type = lib.types.str;
            description = "Plugin source pointer (e.g., github:owner/repo or path:/...).";
          };
          config = lib.mkOption {
            type = lib.types.attrs;
            default = {};
            description = "Plugin-specific configuration (env/files/etc).";
          };
        };
      });
      default = jovLib.effectivePlugins;
      description = "Plugins enabled for this instance (includes first-party toggles).";
    };

    config = lib.mkOption {
      type = lib.types.submodule { options = jovLib.generatedConfigOptions; };
      default = {};
      description = "JOV config (schema-typed).";
    };

    launchd.enable = lib.mkOption {
      type = lib.types.bool;
      default = true;
      description = "Run JOV gateway via launchd (macOS).";
    };

    launchd.label = lib.mkOption {
      type = lib.types.str;
      default = if name == "default"
        then "com.steipete.jov.gateway"
        else "com.steipete.jov.gateway.${name}";
      description = "launchd label for this instance.";
    };

    systemd.enable = lib.mkOption {
      type = lib.types.bool;
      default = true;
      description = "Run JOV gateway via systemd user service (Linux).";
    };

    systemd.unitName = lib.mkOption {
      type = lib.types.str;
      default = if name == "default"
        then "jov-gateway"
        else "jov-gateway-${name}";
      description = "systemd user service unit name for this instance.";
    };

    app.install.enable = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = "Install JOV.app for this instance.";
    };

    app.install.path = lib.mkOption {
      type = lib.types.str;
      default = "${jovLib.homeDir}/Applications/JOV.app";
      description = "Destination path for this instance's JOV.app bundle.";
    };

    appDefaults = {
      enable = lib.mkOption {
        type = lib.types.bool;
        default = name == "default";
        description = "Configure macOS app defaults for this instance.";
      };

      attachExistingOnly = lib.mkOption {
        type = lib.types.bool;
        default = true;
        description = "Attach existing gateway only (macOS).";
      };

      nixMode = lib.mkOption {
        type = lib.types.bool;
        default = true;
        description = "Enable JOV Nix mode in the macOS app via defaults (jov.nixMode).";
      };
    };
  };
}
