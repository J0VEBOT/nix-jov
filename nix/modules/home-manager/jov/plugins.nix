{ lib, pkgs, jovLib, enabledInstances }:
...
let
  ...
  resolvePlugin = plugin: let
    ...
    jovPluginRaw =
      if flake ? jovPlugin then flake.jovPlugin
      else throw "jovPlugin missing in ${plugin.source}";
    jovPlugin =
      if builtins.isFunction jovPluginRaw
      then jovPluginRaw system
      else jovPluginRaw;
    resolvedPlugin =
      if jovPlugin == null
      then throw "jovPlugin is null in ${plugin.source} for ${system}"
      else jovPlugin;
    needs = resolvedPlugin.needs or {};
  in {
    source = plugin.source;
    name = resolvedPlugin.name or (throw "jovPlugin.name missing in ${plugin.source}");
    ...
    lib.mapAttrs (instName: inst:
      ...
      in
        ...
        then ordered
          ...
          "programs.jov.instances.${instName}: duplicate plugin names detected (${lib.concatStringsSep ", " duplicates}); last entry wins."
          ordered
    ) enabledInstances;
    ...
    lib.flatten (lib.mapAttrsToList (instName: inst:
      let
        ...
        mkAssertion = p:
          ...
          in {
            assertion = missing == [];
            message = "programs.jov.instances.${instName}: plugin ${p.name} missing required env: ${lib.concatStringsSep ", " missing}";
          };
        mkConfigAssertion = p: {
          assertion = !(configMissingStateDir p);
          message = "programs.jov.instances.${instName}: plugin ${p.name} provides settings but declares no stateDirs (needed for config.json).";
        };
      in
