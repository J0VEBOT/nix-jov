...
let
  ...
  cfg = jovLib.cfg;
  homeDir = jovLib.homeDir;
  appPackage = jovLib.appPackage;

  defaultInstance = {
    enable = cfg.enable;
    package = jovLib.defaultPackage;
    stateDir = cfg.stateDir;
    workspaceDir = cfg.workspaceDir;
    configPath = "${cfg.stateDir}/jovebot.json";
    logPath = "/tmp/jov/jov-gateway.log";
    gatewayPort = 18789;
    gatewayPath = null;
    gatewayPnpmDepsHash = lib.fakeHash;
    launchd = cfg.launchd;
    systemd = cfg.systemd;
    plugins = jovLib.effectivePlugins;
    config = {};
    appDefaults = {
      enable = true;
    ...
    app = {
      install = {
        enable = false;
        path = "${homeDir}/Applications/JOV.app";
      };
    };
  };
  ...
  plugins = import ./plugins.nix { inherit lib pkgs jovLib enabledInstances; };

  files = import ./files.nix {
    inherit config lib pkgs jovLib enabledInstances plugins;
  };
  ...
  mkInstanceConfig = name: inst: let
    gatewayPackage =
      if inst.gatewayPath != null then
        pkgs.callPackage ../../packages/jov-gateway.nix {
          gatewaySrc = builtins.path {
            path = inst.gatewayPath;
            name = "jov-gateway-src";
          };
    ...
    configFile = pkgs.writeText "jov-${name}.json" configJson;
    gatewayWrapper = pkgs.writeShellScriptBin "jov-gateway-${name}" ''
      set -euo pipefail
      ...
      exec "${gatewayPackage}/bin/jovebot" "$@"
    '';
    ...
    appInstall = if !(pkgs.stdenv.hostPlatform.isDarwin && inst.app.install.enable && appPackage != null) then
      ...
      value = {
        source = "${appPackage}/Applications/JOV.app";
        recursive = true;
        force = true;
      };
    };
  ...
  in {
    homeFile = {
      name = jovLib.toRelative inst.configPath;
    ...
    launchdAgent = lib.optionalAttrs (pkgs.stdenv.hostPlatform.isDarwin && inst.launchd.enable) {
      "${inst.launchd.label}" = {
        ...
        config = {
          ...
          ProgramArguments = [
            "${gatewayWrapper}/bin/jov-gateway-${name}"
            "gateway"
            "--port"
    ...
    systemdService = lib.optionalAttrs (pkgs.stdenv.hostPlatform.isLinux && inst.systemd.enable) {
      "${inst.systemd.unitName}" = {
        Unit = {
          Description = "JOV gateway (${name})";
        };
        Service = {
          ExecStart = "${gatewayWrapper}/bin/jov-gateway-${name} gateway --port ${toString inst.gatewayPort}";
          WorkingDirectory = inst.stateDir;
          Restart = "always";
          RestartSec = "1s";
          Environment = [
  ...
  config = lib.mkIf (cfg.enable || cfg.instances != {}) {
    assertions = [
      {
        ...
        message = "Only one JOV instance may enable appDefaults.";
      }
    ...
    home.file = lib.mkMerge [
      ...
      (lib.optionalAttrs cfg.reloadScript.enable {
        ".local/bin/jov-reload" = {
          executable = true;
          source = ../jov-reload.sh;
        };
      })
    ];

    home.activation.jovDocumentGuard = lib.mkIf files.documentsEnabled (
    ...
    home.activation.jovDirs = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    ...
    home.activation.jovConfigFiles = lib.hm.dag.entryAfter [ "jovDirs" ] ''
      ${lib.concatStringsSep "\n" (map (item: "run --quiet ${lib.getExe' pkgs.coreutils "ln"} -sfn ${item.configFile} ${item.configPath}") instanceConfigs)}
    '';

    home.activation.jovPluginGuard = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
      set -euo pipefail
      ${plugins.pluginGuards}
    '';

    home.activation.jovAppDefaults = lib.mkIf (pkgs.stdenv.hostPlatform.isDarwin && appDefaults != {}) (
      lib.hm.dag.entryAfter [ "writeBoundary" ] ''
        # Nix mode + app defaults (JOV.app)
        /usr/bin/defaults write ai.jov.mac jov.nixMode -bool ${lib.boolToString (appDefaults.nixMode or true)}
        /usr/bin/defaults write ai.jov.mac jov.gateway.attachExistingOnly -bool ${lib.boolToString (appDefaults.attachExistingOnly or true)}
        /usr/bin/defaults write ai.jov.mac gatewayPort -int ${toString (appDefaults.gatewayPort or 18789)}
      ''
    );

    home.activation.jovLaunchdRelink = lib.mkIf pkgs.stdenv.hostPlatform.isDarwin (
      lib.hm.dag.entryAfter [ "linkGeneration" ] ''
        /usr/bin/env bash ${../jov-launchd-relink.sh}
      ''
    );
