{ config, lib, pkgs, jovLib, enabledInstances, plugins }:

let
  cfg = jovLib.cfg;
  resolvePath = jovLib.resolvePath;
  toRelative = jovLib.toRelative;
  toolSets = jovLib.toolSets;
  ...
  renderSkill = skill:
    let
      ...
      ++ lib.optionals (skill ? jovebot && skill.jov != null) [
        "jov:"
        "  ${builtins.toJSON skill.jov}"
      ]
      ++ [ "---" ];
  ...
  skillAssertions =
    ...
    in
      if duplicateNames == [] then [] else [
        {
          assertion = false;
          message = "programs.jov.skills has duplicate names: ${lib.concatStringsSep ", " duplicateNames}";
        }
      ];
  ...
  documentsAssertions = lib.optionals documentsEnabled [
    {
      ...
      message = "programs.jov.documents must point to an existing directory.";
    }
    {
      assertion = builtins.pathExists (cfg.documents + "/AGENTS.md");
      message = "Missing AGENTS.md in programs.jov.documents.";
    }
    {
      assertion = builtins.pathExists (cfg.documents + "/SOUL.md");
      message = "Missing SOUL.md in programs.jov.documents.";
    }
    {
      assertion = builtins.pathExists (cfg.documents + "/TOOLS.md");
      message = "Missing TOOLS.md in programs.jov.documents.";
    }
  ];
      ...
      let
        guardLine = file: ''
          if [ -e "${file}" ] && [ ! -L "${file}" ]; then
            echo "JOV documents are managed by Nix. Please adopt ${file} into your documents directory and re-run." >&2
            exit 1
          fi
        '';
        guardForDir = dir: ''
  ...
  toolsReport =
    if documentsEnabled then
      ...
      in
        pkgs.writeText "jov-tools-report.md" reportText
    else
      null;

  toolsWithReport =
    if documentsEnabled then
      pkgs.runCommand "jov-tools-with-report.md" {} ''
