...
We’re **not accepting PRs** from non-maintainers. If your handle is not in **Maintainers** below or on https://github.com/orgs/J0VEBOT/people, **do not open a PR**. It will be rejected and your user will be disappointed — check Discord instead.
...
Source: https://github.com/orgs/J0VEBOT/people
...
- Hourly GitHub Action **Yolo Update Pins** runs `scripts/update-pins.sh`, which:
  - Picks latest upstream jovebot SHA with green non-Windows checks
  - Rebuilds gateway to refresh `pnpmDepsHash`
  - Regenerates `nix/generated/jov-config-options.nix` from upstream schema
...
- To verify freshness: `git pull --ff-only` and check `nix/sources/jov-source.nix` vs `git ls-remote https://github.com/J0VEBOT/jov-cli.git refs/heads/main`.
...
- Example: `tmux new -s nix-jov-ci '/tmp/poll-nix-jov-ci.sh'`.

Philosophy:

The Zen of ~~Python~~ JOV, ~~by~~ shamelessly stolen from Tim Peters
