...
source_file="$repo_root/nix/sources/jov-source.nix"
app_file="$repo_root/nix/packages/jov-app.nix"
...
upstream_checks_green() {
  local sha="$1"
  local checks_json
  checks_json=$(gh api "/repos/J0VEBOT/jov-cli/commits/${sha}/check-runs?per_page=100" 2>/dev/null || true)
...
log "Resolving jovebot main SHAs"
mapfile -t candidate_shas < <(gh api /repos/J0VEBOT/jov-cli/commits?per_page=10 | jq -r '.[].sha' || true)
if [[ ${#candidate_shas[@]} -eq 0 ]]; then
  latest_sha=$(git ls-remote https://github.com/J0VEBOT/jov-cli.git refs/heads/main | awk '{print $1}' || true)
  if [[ -z "$latest_sha" ]]; then
    echo "Failed to resolve jovebot main SHA" >&2
    exit 1
  fi
...
for sha in "${candidate_shas[@]}"; do
  ...
  source_url="https://github.com/J0VEBOT/jov-cli/archive/${sha}.tar.gz"
  ...
  if ! nix build .#jov-gateway --accept-flake-config >"$build_log" 2>&1; then
...
release_json=$(gh api /repos/J0VEBOT/jov-cli/releases?per_page=20 || true)
if [[ -z "$release_json" ]]; then
  echo "Failed to fetch release metadata" >&2
  exit 1
fi
release_tag=$(printf '%s' "$release_json" | jq -r '[.[] | select([.assets[]?.name | (test("^JOV-.*\\.zip$") and (test("dSYM") | not))] | any)][0].tag_name // empty')
if [[ -z "$release_tag" ]]; then
  echo "Failed to resolve a release tag with an JOV app asset" >&2
  exit 1
fi
log "Latest app release tag with asset: $release_tag"

app_url=$(printf '%s' "$release_json" | jq -r '[.[] | select([.assets[]?.name | (test("^JOV-.*\\.zip$") and (test("dSYM") | not))] | any)][0].assets[] | select(.name | (test("^JOV-.*\\.zip$") and (test("dSYM") | not))) | .browser_download_url' | head -n 1 || true)
if [[ -z "$app_url" ]]; then
  echo "Failed to resolve JOV app asset URL from latest release" >&2
  exit 1
fi
...
log "Regenerating jovebot config options from upstream schema"
tmp_src=$(mktemp -d)
cleanup_tmp() {
  rm -rf "$tmp_src"
}
...
nix shell --extra-experimental-features "nix-command flakes" nixpkgs#nodejs_22 nixpkgs#pnpm_10 -c \
  bash -c "cd '$tmp_src/src' && pnpm exec tsx '$repo_root/nix/scripts/generate-config-options.ts' --repo . --out '$repo_root/nix/generated/jov-config-options.nix'"
...
if [[ "$current_system" == *darwin* ]]; then
  ...
  if ! nix build .#jov-app --accept-flake-config >"$app_build_log" 2>&1; then
...
git add "$source_file" "$app_file" "$repo_root/nix/generated/jov-config-options.nix" "$repo_root/flake.lock"
git commit -F - <<'EOF'
🤖 codex: bump jovebot pins (no-issue)

What:
- pin jovebot source to latest upstream main
- refresh macOS app pin to latest release asset
- update source and app hashes
- regenerate config options from upstream schema

Why:
- keep nix-jov on latest upstream for yolo mode

Tests:
- nix build .#jov-gateway --accept-flake-config
- nix build .#jov-app --accept-flake-config
EOF
